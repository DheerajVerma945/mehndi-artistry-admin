...
import { Pencil, Loader2, Save, Trash2, X, Check, ArrowLeft, Timer } from 'lucide-react'
...
const PostDetails = () => {
  ...
  return (
    <div className="min-h-screen pt-[90px] px-4 py-10 bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto bg-white rounded-2xl shadow-xl p-4 md:p-6 space-y-8">
        ...
        <div className="space-y-6">
          <div>
            <label className="text-sm font-medium text-gray-600">Application Time (minutes)</label>
            {isEditing ? (
              <input
                type="number"
                value={time}
                onChange={(e) => setTime(e.target.value)}
                className="w-full mt-1 px-4 py-2 rounded-md bg-gray-100 border border-gray-300 focus:ring-2 focus:ring-black focus:outline-none"
                min="1"
              />
            ) : (
              <p className="text-gray-800 font-medium mt-1 flex items-center gap-2">
                <Timer className="w-4 h-4" /> {post.time} min
              </p>
            )}
          </div>
